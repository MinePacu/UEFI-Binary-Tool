...
set "missing_files="
if not exist "msi_main.py" set "missing_files=%missing_files% msi_main.py"
if not exist "msi\analyzer\msi_analyzer.py" set "missing_files=%missing_files% msi\analyzer\msi_analyzer.py"
if not exist "msi\repacker\msi_repacker.py" set "missing_files=%missing_files% msi\repacker\msi_repacker.py"
...
:run_repacker
set "extract_dir=%PROJECT_ROOT%\msi_extracted"
if not exist "%extract_dir%" (
    echo %EXTRACT_MISSING% "%extract_dir%"
    echo.
    echo %EXTRACT_HINT%
    echo %PROJECT_ROOT_LABEL% "%PROJECT_ROOT%"
    echo.
    pause
    goto :eof
)
echo.
echo %REPACK_MODE%
echo %REPACK_TARGET% "%extract_dir%"
echo.
set PYTHONIOENCODING=utf-8
...
echo - *_msi_analysis_report.txt
echo - *_msi_repacked.bin
echo - msi_extracted/
echo.
